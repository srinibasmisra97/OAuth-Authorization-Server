...
URL = str(cfg.get(CONFIG_ENV, "URL")) + "/gtypes"
...
@pytest.mark.app_gtypes
def test_app_gtypes_request_method():
    """
...
@pytest.mark.app_gtypes
def test_app_gtypes_auth_header():
    """
...
@pytest.mark.app_gtypes
def test_app_gtypes_content_type():
    """
...
@pytest.mark.app_gtypes
def test_app_gtypes_parameters():
    """
    No API id provided.
    """
    ...
    """
    No grant_types provided.
    """
    response = requests.put(URL, headers=HEADERS, json={'api': APP_API})
    assert response.status_code == 400, "Invalid status code for no gtypes provided"


@pytest.mark.run(order=14)
@pytest.mark.app_gtypes
def test_app_gtypes_update():
    """
    Update grant types.
    """
    HEADERS = {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + get_token()}
    response = requests.put(URL, headers=HEADERS, json={'api': APP_API, 'grant_types': APP_GRANT_TYPE})
    assert response.status_code == 200, "Error while updating grant types"
